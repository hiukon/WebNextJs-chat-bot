...
import type { IMessage } from "./useChat";
...
function MessageItem({ m }: { m: IMessage }) {
    // chỉ compute time trên client để tránh hydration mismatch
    const [timeText, setTimeText] = useState<string | null>(null);

    useEffect(() => {
        setTimeText(new Date(m.createdAt).toLocaleTimeString());
    }, [m.createdAt]);

    return (
        <div
            className={`${styles.message} ${m.role === "user" ? styles["user-message"] : styles["bot-message"]} ${m.text === "..." ? styles["thinking"] : ""}`}
        >
            {m.role !== "user" && (
                <svg className={styles["bot-avatar"]} xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 650 700">
            ...
            <div className={styles["message-content"]}>
                ...
                {m.image && <img src={m.image} alt="attachment" className={styles.attachment} />}

                {/* hidden meta to match minimal look */}
            </div>
        </div>
    );
}
