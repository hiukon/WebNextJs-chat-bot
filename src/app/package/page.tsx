...
import AntdMenuBox from './app.optionbox';
import Link from 'next/link';

const fetcher = (url: string) => fetch(url).then(res => res.json());

const MenuBoxes = () => {
    const { data, error, isLoading } = useSWR<IBox[]>('http://localhost:3000/api/boxes', fetcher);
    ...
    const filteredBoxes = selectedCategory === 'Tất cả'
        ? data
        : data.filter(b => b.category === selectedCategory);

    const totalPages = Math.ceil(filteredBoxes.length / ITEMS_PER_PAGE);
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    const currentBoxes = filteredBoxes.slice(startIndex, startIndex + ITEMS_PER_PAGE);
                        ...
                        {currentBoxes.map((b) => (
                            ...
                            >
                                ...
                                <button className="p-1 ml-5 text-green-700 text-left w-full">
                                    <p>{b.name}</p>
                                    <p className="font-semibold">{b.price.toLocaleString()}đ</p>
                                </button>
